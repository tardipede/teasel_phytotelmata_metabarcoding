...
if (MARKER == "18S") {
  taxonomy_ranks <- c("Root", "Domain", paste0("Rank_", 1:17))
}
if (MARKER == "16S") {
  taxonomy_ranks <- c("Root", "Domain", "Phylum", "Class", "Order", "Family", "Genus")
}

paths <- c(list.files("./intermediates", 
                      pattern = PREFIX, 
                      full.names = TRUE), 
           list.files("./samples_data", 
                      full.names = TRUE))

euk_phyloseq_dataset <- make_phyloseq(paths = paths, 
                                      taxonomy_ranks = taxonomy_ranks) %>%
  saveRDS_pipe(filename = paste0("./intermediates/", 
                                 OUT_NAME, "_01_all_reads.rds")) %>%
  subtract_blank(blank_name = "DFP.000") %>%
  prune_samples(sample_sums(.) != 0, .) %>%
  phyloseq::subset_taxa(Domain != "unclassified_Root") %>% # Remove uncl seqs
  phyloseq::subset_taxa(Rank_6 != "Embryophyta") %>% # Remove embriophytes
  saveRDS_pipe(filename = paste0("./intermediates/", 
                                 OUT_NAME, 
                                 "_02_noblank_phyloseq.rds")) %>%
  ...
  saveRDS_pipe(filename = paste0("./intermediates/", 
                                 OUT_NAME, "_03_rarefied_phyloseq.rds"))
...
bac_phyloseq_dataset <- make_phyloseq(paths = paths, 
                                      taxonomy_ranks = taxonomy_ranks) %>%
  saveRDS_pipe(filename = paste0("./intermediates/", 
                                 OUT_NAME, "_01_all_reads.rds")) %>%
  subtract_blank(blank_name = "DFP.000") %>%
