...
# Plot Jaccard
plot_jac = data.frame(ord_jac$points); plot_jac$sample = rownames(plot_jac)
...
plot_all_jac = ggplot(plot_jac)+
  theme_bw()+
  geom_path(aes(x = MDS1, y=MDS2, col = as.factor(plant), group =  as.factor(plant)))+
  geom_point(aes(x = MDS1, y=MDS2, shape = as.factor(level), fill = as.factor(plant)), size = 4,color = "black", alpha = 0.75)+
...
# Plot Turnover
plot_tur = data.frame(ord_tur$points); plot_tur$sample = rownames(plot_tur)
...
ggsave("./results/figures/plot_NMDS.pdf", plot = plot_nmds, height = 10, width = 14)