...
# EUK
asv_table <- data.frame(otu_table(euk_dataset))
asv_table <- cbind(data.frame(ASV = rownames(asv_table)), asv_table)
tax_table <- data.frame(tax_table(euk_dataset))
tax_table <- cbind(data.frame(ASV = rownames(tax_table)), tax_table)
...
# BAC
asv_table <- data.frame(otu_table(bac_dataset))
asv_table <- cbind(data.frame(ASV = rownames(asv_table)), asv_table)
tax_table <- data.frame(tax_table(bac_dataset))
tax_table <- cbind(data.frame(ASV = rownames(tax_table)), tax_table)
