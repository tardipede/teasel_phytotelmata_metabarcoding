...
if (length(args) != 5) {
  ...
  provide RUN_NAME [arg1], 
  DATA_FOLDER [arg2], 
  MARKER [arg3](18S or 16S), 
  RUN_REGEX [arg4] and 
  SAMPLE_REGEX [arg5]")
}
...
sample_table <- tibble::tibble(
  ...
  sample = stringr::str_extract(fastq_R1, SAMPLE_REGEX)
) # Extract sample code
...
CUTADAPT_FLAGS <- paste0("cutadapt -a ^", 
                         FWD, 
                         "...", 
                         REV.RC, 
                         " -A ^", 
...
out <- filterAndTrim(sample_table$trim_R1, 
  ...
  compress = TRUE, multithread = TRUE, minLen = 190
)
...
for (i in 1:length(unique(sample_table$runcode))) {
  ...
  errF <- learnErrors(sample_table_temp$filt_R1, 
                      multithread = TRUE, nbases = 1e+8, randomize = TRUE)
  errR <- learnErrors(sample_table_temp$filt_R2, 
                      multithread = TRUE, nbases = 1e+8, randomize = TRUE)
  # Sample inference
  dadaFs <- dada(sample_table_temp$filt_R1, err = errF, multithread = TRUE)
  dadaRs <- dada(sample_table_temp$filt_R2, err = errR, multithread = TRUE)
...
system(paste0("makeblastdb -in ", 
              temp_blast_path, 
              "/seqs.fasta -parse_seqids -dbtype nucl -out ", 
              temp_blast_path, 
              "/db"))

# Then blast the OTUs against the database (could be run with DIAMOND to make it faste?)
system(paste0("blastn -db ", 
              temp_blast_path, 
              "/db -outfmt 6  -out ", 
              temp_blast_path, 
              "/match_list.txt -qcov_hsp_perc 95 -perc_identity 95 -query ", 
              temp_blast_path, 
              "/seqs.fasta"))

# Load blast results
blast_res <- read.table(file.path(temp_blast_path, 
                                  "match_list.txt"), 
                        header = FALSE, sep = "\t")
# Remove blast folder
...
# Run lulu
curated_result <- lulu(otutable_lulu, 
                       blast_res, 
                       minimum_match = 99, 
                       minimum_relative_cooccurence = 1,
                       minimum_ratio = 10, 
                       log_directory = paste0(seq_path, "/"))
...
saveRDS(list(curated_result = curated_result, dna = dna), 
        file.path(backup_path, "savepoint2_afterlulu.rds"))
# Read the sequence table (only a part of the script can be run from here)
...
write.table(otutable_classified, 
            file = paste0("./intermediates/", RUN_NAME, "_classified_otus.csv"), 
            quote = FALSE, 
            sep = "\t")
...
Biostrings::writeXStringSet(aln, 
                            paste0("./intermediates/", 
                                   RUN_NAME, "_aligned_otus.fas"))


# Make OTUs phylogenetic tree with Fastree
system(paste0("FastTree -gtr -nt ./intermediates/", 
              RUN_NAME, 
              "_aligned_otus.fas > ./intermediates/", 
              RUN_NAME, 
              "_otus_phylotree.tre"))
